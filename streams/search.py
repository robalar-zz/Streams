...
def start_search(searchTerm, page):

	search_object = _Search(searchTerm, page)
...
class _Search(object):
	...
	def Request(self, parameters):
		...
		try:
			r = requests_session.get(url='https://yts.to/api/v2/list_movies.json', params=parameters, proxies=streams.PROXIES)
	...
	def parse_data(self, rawData):
		#read json data
		json_data = json.loads(rawData.text)
	...
	def populate_movie_list(self, data):
		for m in data['movies']:
			if m['state'] == 'DMCA Removed':
				...
				movie_object = movie.create_movie(title=m['title'], genres=m['genres'], 
				torrents=m['torrents'], url=m['url'], rating=m['rating'], age_rating=m['mpa_rating'], length=m['runtime'])
	...
	def __init__(self, searchTerm, page):
		...
		self.page_count = int(math.ceil(self.movie_count/self.results_per_page) + 1)
...
def search_to_html(search_object):
	...
	for movie in search_object.movies:
		row = tbody.tr()
		row.td(movie.title)
		row.td(str(movie.length))
		row.td(movie.age_rating)
